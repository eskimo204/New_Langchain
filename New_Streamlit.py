...
def img_prompt_func(data_dict):
    ...
    text_message = {"type": "text", "text": f'{formatted_texts}\n{data_dict["question"]}'}
    messages.append(text_message)

    return messages

def multi_modal_rag_chain(retriever):
    text_qa_prompt = """Use the following pieces of context to answer the question at the end. 
    If you don't know the answer, just say you don't know. DO NOT try to make up an answer.
    {context} Question: {question}"""

    prompt_template = PromptTemplate(template=text_qa_prompt, input_variables=["context", "question"])

    chain = (
        {"context": RunnablePassthrough(), "question": RunnablePassthrough()}
        | RunnableLambda(split_image_text_types)
        | ChatPromptTemplate.from_messages(img_prompt_func)
        | ChatOpenAI(model="gpt-4", max_tokens=2048)
        | StrOutputParser()
    )
